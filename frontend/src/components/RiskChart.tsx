...
export function RiskChart({ selectedRepo }: ChartProps) {
  const { data: stats, isLoading } = useStats(selectedRepo);
...
export function TrendChart({ selectedRepo }: ChartProps) {
  const [range, setRange] = useState<TimeRange>("daily");
  const { data: issues, isLoading } = useReviews(selectedRepo);
